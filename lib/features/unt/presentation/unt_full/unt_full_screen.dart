  ...
  Widget build(BuildContext context) {
    return Scaffold(
                          ...
                          Center(
                            child: LiteRollingSwitch(
                              ...
                              onChanged: (bool state) {
                                context.read<UntFullBloc>().add(UntFullUpdateLocaleIdEvent(state ? 1 : 2));
                              },
                              onTap: () {},
                              onDoubleTap: () {},
                              onSwipe: () {},
                        ...
                        SliverGrid(
                            delegate: SliverChildBuilderDelegate(
                              ...
                              return GestureDetector(
                                onTap: (){
                                  List<int> newValue =  List<int>.from(state.parameter?.subjects??[]);
                                  if(newValue.contains(state.subjects[index].id)){
                                    ...
                                    if(newValue.length >= 2){
                                      ...
                                      newValue[1] = state.subjects[index].id;
                                    }
                                    else{
                                      newValue.add(state.subjects[index].id);
                                    }
                                  }
                                  context.read<UntFullBloc>().add(UntFullAddSubjectsEvent(newValue));
                                },
                                child: Container(
                                  decoration: BoxDecoration(
                                  ...
                                  child: Stack(
                                    ...
                                    children: [
                                       Positioned(
                                        right: 10,
                                        top: 5,
                                        child: (state.parameter?.subjects.contains(state.subjects[index].id) == true ?
                                        const Icon(FontAwesomeIcons.circleCheck,
                                            color: Colors.lightGreenAccent) : SizedBox()),
                                      ),
                                      Positioned(
                                          bottom: 20,
                                          child: Container(
      ...
      floatingActionButton: BlocBuilder<UntFullBloc, UntFullState>(
        builder: (context, state) {
          if (state is UntFullSubjectLoadedState) {
            if(state.parameter?.subjects.length == 2 && state.parameter?.locale_id != null && state.parameter?.attempt_type_id == 1){
