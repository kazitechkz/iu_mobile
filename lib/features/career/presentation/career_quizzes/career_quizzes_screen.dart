...
import 'package:iu/features/career/presentation/career_quizzes/widget/load_more_quizzes_button_widget.dart';
...
class _CareerQuizzesScreenState extends State<CareerQuizzesScreen> {
  ...
  void initState() {
    super.initState();
    context
        .read<CareerQuizzesBloc>()
        .add(GetCareerQuizzesEvent(GetCareerQuizzesParameter(page: 1)));
  }
            ...
            return SingleChildScrollView(
              child: Column(
                children: [
                      ...
                      Expanded(
                        child: Padding(
                          ...
                          child: InkWell(
                            ...
                            onTap: () {
                              context.go(
                                  "/${RouteConstant.careerSubscriptionsName}");
                            },
                            child: Container(
                      ...
                      Expanded(
                        child: Padding(
                          ...
                          child: InkWell(
                            ...
                            onTap: () {
                              context.go(
                                  "/${RouteConstant.myCareerQuizAttemptsName}");
                            },
                            child: Container(
                              decoration: BoxDecoration(
                                borderRadius: BorderRadius.circular(50.w),
                  ...
                  ..._getCareerCardWidget(
                      state.careerQuizzes, state.careerQuizzesEntity.purchased),
                  (state.isLoadingPagination
                      ? Center(
                          child: CircularProgressIndicator(),
                        )
                      : (state.careerQuizzesEntity.quizzes.lastPage >
                              state.careerQuizzesEntity.quizzes.currentPage
                          ? LoadMoreQuizzesButton(
                              page: state
                                      .careerQuizzesEntity.quizzes.currentPage +
                                  1)
                          : SizedBox()))
                ],
              ),
  ...
  List<Widget> _getCareerCardWidget(
      List<CareerQuizEntity> quizzes, List<int> purchased) {
    return quizzes
        .map((careerQuiz) =>
            CareerQuizCardWidget(quizEntity: careerQuiz, purchased: purchased))
        .toList();
  }
}
