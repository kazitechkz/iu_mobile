...
class _AttemptResultScreenState extends State<AttemptResultScreen> {
          ...
          if (state is AttemptResultSuccessState) {
            return SingleChildScrollView(
              child: Padding(
                ...
                child: Column(
                  children: [
                    AttemptResultCardWidget(attemptEntity: state.stats.attempt,),
                    SizedBox(height: 15.h,),
                    ..._getSubjectCard(state.stats.subjects,state.stats.subjectResult)
                  ],
                )
              ),
            );
  ...
  List<Widget> _getSubjectCard(List<SubjectEntity>? subjects,List<SubjectResultEntity>? subjectResults){
    List<Widget> widgets = [];
    if(subjects != null){
      widgets = subjects.map((subjectItem){
        final subjectResult = subjectResults?.firstWhereOrNull((subjectResultEntity) => subjectResultEntity.subjectId == subjectItem.id);
        return AttemptResultSubjectCard(subjectEntity: subjectItem, subjectResultEntity: subjectResult);
      }).toList();
    }
